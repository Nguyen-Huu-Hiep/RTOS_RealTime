...
public class RTSJ_Robot {
    ...
    static class RobotConfig {
        ...
        private final int[] sensorIdsPerClient; // Mang ID cam bien cho moi client
        ...
        private RobotConfig(Builder builder) {
            ...
            this.sensorIdsPerClient = builder.sensorIdsPerClient;
        ...
        public static class Builder {
            ...
            private int[] sensorIdsPerClient = {0}; // Mac dinh client co 1 cam bien
            ...
            public Builder sensorIdsPerClient(int[] ids) { this.sensorIdsPerClient = ids; return this; }
            ...
            public RobotConfig build() {
                if (serverPort <= 0 || sensorIdsPerClient.length == 0 || sensorSleepTime <= 0 ||
        ...
        public int[] getSensorIdsPerClient() { return sensorIdsPerClient; }
    ...
    static class ClientSensorTask implements Runnable {
        ...
        private final int sensorId; // 0: sieu am truoc, 1: bui, 2: do am
        ...
        public void run() {
            while (true) {
                ...
                System.out.println(); // Gian cach sau moi chu ky cam bien
                try {
        ...
        private int readSensor() {
            switch (sensorId) {
                case 0: return (int) (Math.random() * 100); // Cam bien sieu am truoc (khoang cach cm)
                case 1: return (int) (Math.random() * 50);  // Cam bien bui (muc do bui 0-50)
                case 2: return (int) (Math.random() * 100); // Cam bien do am (phan tram 0-100)
                default: return 0;
            }
        }
    }
    ...
    static class ControlTask implements Runnable {
        ...
        public void run() {
            ...
            System.out.println(); // Gian cach sau moi chu ky xu ly
            nextTask.notifyTask();
        }

        private int processSensorData() {
            if (sensorValues.isEmpty()) return 0; // Tien len neu khong co du lieu
            Integer frontDistance = sensorValues.getOrDefault(0, Integer.MAX_VALUE); // Cam bien sieu am truoc
            Integer dustLevel = sensorValues.getOrDefault(1, 0);                    // Cam bien bui
            Integer humidity = sensorValues.getOrDefault(2, 0);                     // Cam bien do am

            // Chuyen che do dua tren do am va bui
            if (humidity > 70) mode.set(1); // Lau san neu do am cao
            else if (dustLevel > 30) mode.set(0); // Hut bui neu bui nhieu

            // Logic dieu khien dua tren khoang cach truoc
    ...
    static class MotorTask implements Runnable {
        ...
        public void run() {
            ...
            System.out.println(); // Gian cach sau moi chu ky dong co
        }
            ...
            try (BufferedReader in = new BufferedReader(new InputStreamReader(clientSocket.getInputStream()));
                ...
                while (true) {
                    ...
                    if (data != null) {
                        ...
                        System.out.println(); // Gian cach sau moi chu ky xu ly client
                    }
                }
            } catch (IOException e) {
    ...
    static class ClientTask implements Runnable {
        ...
        public void run() {
            try (Socket socket = new Socket("localhost", port);
                ...
                // Khoi tao cac thread cam bien theo config
                ExecutorService sensorExecutor = Executors.newFixedThreadPool(config.getSensorIdsPerClient().length);
                for (int sensorId : config.getSensorIdsPerClient()) {
                    ClientSensorTask sensorTask = new ClientSensorTask(name + "-Sensor-" + sensorId, sensorId, out);
                ...
                while (true) {
                    ...
                    if (status != null) {
                        ...
                        System.out.println(); // Gian cach sau moi chu ky nhan du lieu
                    }
                }
            } catch (IOException e) {
    ...
    public static void main(String[] args) {
        // Khoi tao cau hinh cho server
        RobotConfig configClient1 = new RobotConfig.Builder()
            .serverPort(12345)
            .sensorIdsPerClient(new int[]{0}) // Client 1: Cam bien sieu am truoc
            .sensorSleepTime(100)
            .controlSleepTime(50)
            .motorSleepTime(50)
            .build();

        RobotConfig configClient2 = new RobotConfig.Builder()
            .serverPort(12345)
            .sensorIdsPerClient(new int[]{1, 2}) // Client 2: Cam bien bui va do am
            .sensorSleepTime(100)
            .controlSleepTime(50)
            .motorSleepTime(50)
            .build();
        ...
        ServerTask serverTask = new ServerTask("ServerTask", configClient1.getServerPort(), controlTask);

        scheduler.scheduleAtFixedRate(controlTask, 0, configClient1.getControlSleepTime(), TimeUnit.MILLISECONDS);
        scheduler.scheduleAtFixedRate(motorTask, 0, configClient1.getMotorSleepTime(), TimeUnit.MILLISECONDS);

        // Khoi dong server va nhieu client voi socket rieng
        Executors.newSingleThreadExecutor().submit(serverTask);
        Executors.newSingleThreadExecutor().submit(new ClientTask("ClientTask1", configClient1.getServerPort(), configClient1));
        Executors.newSingleThreadExecutor().submit(new ClientTask("ClientTask2", configClient2.getServerPort(), configClient2));
    }
}